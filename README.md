...
- 📅 Выбор периода (From–To) + быстрые пресеты: **3 дня / 1 неделя / 2 недели / 1 месяц / всё время**
- 📊 Интерактивный график (Plotly):
  - глюкоза — сглаженная линия + подсветка целевого диапазона
  - инсулин — столбики (болюс/базал)
...
## Запуск (рекомендуется) — через Makefile
...
## Запуск без Makefile (Windows)

```bash
python -m venv env
env\Scripts\pip.exe install -r requirements.txt
env\Scripts\streamlit.exe run healthkit_diabetes.py
```
