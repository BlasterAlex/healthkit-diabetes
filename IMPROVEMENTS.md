...
- [x] **Сводная статистика** — блок с ключевыми показателями за выбранный период:
  - среднее/мин/макс глюкозы
  - суммарный инсулин (базальный + болюсный)
  - суммарные углеводы
  - % времени в целевом диапазоне (Time in Range)
- [x] **Настраиваемые пороги глюкозы** — `GLUCOSE_MIN` / `GLUCOSE_MAX` вынесены в константы в начало файла (UI-слайдер — следующий шаг)
- [x] **Почасовые метки на оси X** — динамический интервал меток в зависимости от диапазона: 2 ч / 6 ч / 12 ч / 24 ч / 7 дней
